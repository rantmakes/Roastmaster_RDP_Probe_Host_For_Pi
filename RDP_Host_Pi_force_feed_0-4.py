...
class MockProbeHost:
    ...
    def send_syn(self):
        ...
        # Manually build SYN payload too (just in case)
        # It's a single item so commas don't matter, but consistency helps
        payload_str = json.dumps(payload_array)
    ...
    def send_temps(self):
        ...
        for item in sensor_data:
            ...
            # Serialize each event individually to a string
            payload_items.append(json.dumps(event))

        current_epoch = time.time()

        # --- MANUAL STRING CONSTRUCTION ---
        # Join the serialized items with a SPACE instead of a COMMA
        # Result: [{"a":1} {"b":2}]
        custom_payload_str = "[" + " ".join(payload_items) + "]"

        datagram = {
            KEY_VERSION: RDP_VERSION_1_0,
            KEY_SERIAL: HOST_SERIAL,
            KEY_EPOCH: current_epoch,
            # Use our manually constructed string
            KEY_PAYLOAD: custom_payload_str 
        }

        # Note: The outer datagram is still valid JSON
        msg_bytes = json.dumps(datagram).encode('utf-8')
        self.write_web_log(datagram)
    ...
    def run(self):
        print(f"Roastmaster NO-COMMA Host Started.")
