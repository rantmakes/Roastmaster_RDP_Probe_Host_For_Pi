...
# MATCHES YOUR ROASTMASTER APP SETTING
...
SYNC_SEND_RATE = 2.0
TEMP_SEND_RATE = 0.5 
FORCE_START_DELAY = 6.0 # Seconds to wait before Blind Firing data
...
class MockProbeHost:
    ...
    def read_incoming(self):
        # We still listen, just in case the ACK actually shows up
        try:
            ...
            if (packet.get(KEY_VERSION) == RDP_VERSION_1_0 and 
                ...
                self.start_time = 0 # Cancel the force timer
    ...
    def send_temps(self):
        ...
        for item in sensor_data:
            # FIX: Send as raw Float, rounded to 2 decimals
            # This ensures it appears as 200.55 in JSON, not "200.55"
            val_float = round(float(item["val"]), 2)
            
            event = {
                KEY_EVENT_TYPE: EVENT_TEMP, 
                KEY_CHANNEL: item["channel"],
                KEY_VALUE: val_float, # Sending NUMBER (Float)
                KEY_META: item["meta"]
            }
        ...
        datagram = {
            ...
            KEY_SERIAL: HOST_SERIAL,
            KEY_EPOCH: current_epoch,
            # We keep the Double Encoding for the payload container
            # But inside this string, RPValue will be a true number
            # json.dumps AUTOMATICALLY adds commas between list items.
        ...
        # IF we have a server address (ACK received), use it.
        # IF NOT, blind fire to the Multicast Group.
    ...
    def run(self):
        print(f"Roastmaster BLIND FIRE Host Started.")
        print(f"Serial: {HOST_SERIAL}")
        print(f"Targeting Multicast Group: {MULTICAST_GROUP}")
        ...
        while True:
            ...
            # --- FORCE START LOGIC ---
            # If we are still SEARCHING after the delay, force CONNECTED state
            if self.state == HostState.SEARCHING and (current_time - self.start_time > FORCE_START_DELAY):
                print("\n\n*** TIMEOUT: Forcing Connection Mode (Blind Fire) ***")
                self.state = HostState.CONNECTED
